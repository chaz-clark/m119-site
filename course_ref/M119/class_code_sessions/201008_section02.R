...
f.exp <- function(x,a=1,b=2,c=0,d=0){
  a*b^(x+c)+d
}
...
#Use the lines() function to add a line (or curve) to the current plot.
  #Try the specific model with parameters a=1,b=2,c=-3,d=-1.
lines(x,f.exp(x,a=1,b=2,c=-3,d=-1),col=2)
  #Try the specific model with parameters a=1,b=2,c=0,d=0.
  #Change the color of the curve with the col=3 argument so we can tell the specific models apart.
lines(x,f.exp(x,a=1,b=2,c=0,d=0),col=3)
  #Try the specific model with parameters a=1/3,b=2,c=-3,d=0.
  #Change the color of the curve with the col=4 argument so we can tell the specific models apart.
lines(x,f.exp(x,a=1/3,b=2,c=-3,d=0),col=4)
  #Try the specific model with parameters a=1/3,b=2,c=0,d=-1.
  #Change the color of the curve with the col=4 argument so we can tell the specific models apart.
lines(x,f.exp(x,a=1/3,b=2,c=0,d=-1),col=5)
  #Compare with the parameters that were used to create the data.
lines(x,f.exp(x,a=1/5,b=2,c=-1,d=0),col=6)
...
x <- seq(-10,10,0.01)
lines(x,f.quad(x,a=-2,b=0,c=-10),col=2)
lines(x,f.quad(x,a=-2,b=0,c=0),col=3)
lines(x,f.quad(x,a=-2.2,b=0,c=0),col=4)


#Let's create some measurement data using the Exponential distribution, our f_3 function.
  #Pick 1 random number between 0.0000001 and 5 for the true parameter value.
  #Don't look at the value in the Environment tab yet!
l <- runif(1,0.0000001,5)
...
x <- seq(0,1.5,0.01)
  #lambda = 3.72 (rate = 3.72) looks like a decent visual fit.
lines(x,dexp(x,rate=3.72),col=2)
  #lambda = 1 (rate = 1) looks like a bad fit.
lines(x,dexp(x,rate=1),col=3)


#What was the value of lambda? 
  #Note: Because we randomly selected a number between 0.0000001 and 5 when you run this code again you will get a differnent value for l.
  #We could have reproducable code if we set the seed, which means we would all be drawing the same random number between 0.0000001 and 5.
l
#[1] 4.852296
  #Our guess of 3.72 was kind of close.
