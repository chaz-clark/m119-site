...
URL_shiny_p2_t3 <- 'https://posit.byui.edu/content/27bbcd33-a129-45fa-9de1-9355ea44a7b7'
URL_shiny_p2    <- 'https://posit.byui.edu/content/8cba223a-161c-4303-8733-189791271bad'
```
...
- **CHECK YOUR WORK:** 
  ...
  - Use this [Shiny App](`r URL_shiny_p2_t3`) to verify that your parameters are correct. 
...
The critical point for $\ell_2$ is $(a_1,a_2) = (`r best_a1`, `r best_a2`)$. 
...
We see that $D = `r D`$, which means $D>0$. 
The fact that $D>0$, together with $\frac{\partial^2\ell_2}{\partial a_1^2} = `r -c11`<0$ means that our critical point corresponds to a local maximum (by the second derivative test). 
The completes the computations for the maximum likelihood method. 

Our best fit model is $$f_2(t) = 100 + (`r best_a1`)t +(`r best_a2`)t^2$$ where $t \geq 0$. 
...
Because $D = `r D`>0$ and $\frac{\partial^2\ell_2}{\partial a_1^2} = `r -c11`<0$, we know that our critical point corresponds to a local maximum (by the second derivative test). 
The completes the computations for the maximum likelihood method. 

Our best fit model is $$f_6(t) = 100 + (`r best_a1`)t +(`r best_a2`)(1 - e^{-0.0003t})$$ where $t \geq 0$.
Let's confirm this fit visually with the following R code.
...
- Answer the question, "How long does an LED light bulb last?" 
    ...
    - Use the `uniroot()` function in R to find the approximate solution for where each of your five fitted models is at 80% of the initial intensity. So solve the equation $f_i(t) = 80$ for each of the five fitted models.
        - **CHECK YOUR WORK:** Use this [Shiny App](`r URL_shiny_p2`) to check your answers.
