...
- Use the code below to read in the soil data.
    ```{r load_libraries, class.source="fold-show"}
...
- Set the sample mean of the Nitroglycerin data equal to the expected value of $f_0$, set the sample variance of the Nitroglycerin data equal to the variance of $f_0$, and solve the system of equations for $a$ and $b$. Then state the fitted model, $f_0(m)$, with the parameters values rounded to 3 decimal places as needed.
- Set the sample mean of the Nitroglycerin data equal to the expected value of $f_3$ and solve for $\lambda$. Then state the fitted model, $f_3(m)$, with the parameters values rounded to 3 decimal places as needed.
- The fitted models $f_1$ and $f_2$ are $f_1(m) = \frac{1}{\sqrt{2\pi (10.979)}}e^{-\frac{1}{2}\left(\frac{x-2.792}{3.313}\right)^2}$ where $-\infty < m < \infty$ and $f_2(m) = \frac{0.254^{0.710}}{\Gamma(0.710)}m^{0.710-1}e^{-0.254 m}$ where $m \geq 0$. 
...
- Answer the question, "What is the amount of explosives in the soil?"
    ...
    - Provide the fitted models.
        - The work to fit $f_1(t)$ and $f_2(t)$ was completed in class but results from class should be included.
