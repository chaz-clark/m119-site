...
server <- function(input, output) {
  fn <- function(x,p1,p2,type){
    switch (type,
            f1 = 100 + p1*x,
            f2 = 100 + p1*x + p2*x^2,
            f4 = 100 + p1*x + p2*log(0.005*x+1),
            f5 =(100 + p1*x)*exp(-00005*x),
            f6 = 100+p1*x+p2*(1-exp(-0.0003*x))
    )
  }
  ...
  my_message <- eventReactive(input$check_sol, {
    ...
    if(got_solution == "No"){
      ans <- my_tryCatch(
        uniroot(function(x){fn(x,p1=a1,p2=a2,type = my_type)-L},c(no_a,no_b))$root
      )
    ...
    ans <- my_tryCatch(
      uniroot(function(x){fn(x,p1=a1,p2=a2,type = my_type)-L},c(my_a,my_b))$root
    )
    if(is.null(ans)){
  ...
  my_plots <- eventReactive(input$check_sol, {
    ...
    lines(plot_x,fn(plot_x,p1=a1,p2=a2,type = my_type), col=2)
