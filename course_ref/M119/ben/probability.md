---
title: "Rectangles, Rugs, and Riemann Sums"
...
Having made the observation above, let's compute the probability $P(X\leq 2)$. We'll do this in two ways. 
  ...
  * To have $X\leq 2$, we can have $X$ be 0, 1, 2. The corresponding probabilities are $\frac{1}{8}$, $\frac{3}{8}$, $\frac{3}{8}$. Summing these gives $P(X>0)=\frac{7}{8}$. We could also think of this as summing the areas of the three rugs located above 0,1, and 2.
...
We'll look at another example related to the tropical storm example we did earlier this semester. Let $X$ represent the number of tropical storms in a year that make landfall in Florida. We already saw that this discrete random variable follows a Poison distribution $f(x;\lambda) =\lambda^x \frac{e^{-\lambda}}{x!}$, and we estimated the parameter to be $\lambda \approx 5.904762$.  The follow code computes and graphs the first 20 values of the probability mass function (it only displays the first 10 in the table).  
...
1. Use the $x$-vales from 0 to 20 to estimate $E[X]$.
    ...
    b. Adjust the code to use the $x$-vales from 0 to 30 to estimate $E[X]$.
    c. Adjust the code to use the $x$-vales from 0 to 50 to estimate $E[X]$.
...
2. Compute $\mu_X$, the expected value of $X$. 
...
3. Compute $E[X]$, the expected value of $X$, which is the centroid from the previous part. 
4. Compute $F(2)$ and then $F(5)$.  Use these values to compute $P(2<x\leq 5)$.

# Rugs for Continous Random Variables
## A Rectangular Rug
...
1. Change the height of the rug from 2 to 3. Then verify that each of the computations above result in the same probability. 
1. If the area of the rug is 1, then we can skip dividing by area when computing probabilities. The function describing the top of the rug is currently $f(x) = 2$, and the total area under the rug is $A=10$. We can multiply a function by a constant $k$ so that the rug under $kf(x)$ has area 1. We'll call this a **normalized** rug function. Explain why we want $k=1/10$ in this problem, which means our normalized rug function is $f(x) = \frac{1}{10}2 = \frac{1}{5}$ for $0\leq x\leq 5$. 
...
1. From the above computations, we have $F(x)=\begin{cases}0&x<0\\x/5&0\leq x\leq 5\\1&x>5\end{cases}$.  Compute the derivative of $F(x)$. Then compare $F'(x)$ to the normalized rug function.
1. Imagine that we dropped 1000 darts on this rug, and recorded each of their $x$ values. What would the mean $x$-value be?  Explain. 
...
```
##  [1] 2.48336208 0.22453937 0.24711158 3.79610438 1.27175705 0.91989992
##  [7] 1.85086107 0.06092761 3.68278393 1.34317403
```
...
Let $f(x)=\begin{cases}5 & 0\leq x\leq 6\\ 0 &\text{otherwise}\end{cases}$. Consider a rug that lies under this function and above the $x$-axis.  Let $X$ represent the random variable that we obtain by randomly dropping a dart on the rug and recording the $x$-value. 

1. Draw the rug function $f(x)$. 
1. Geometrically explain why $E[X]=3$. 
1. Find a constant $k$ so that the area under $kf(x)$ equals 1. In other words, find the normalized rug function. 
1. Compute $F(2.5)$, and $F(4)$.  Then compute $P(2.5<X\leq 4)$. 
1. Show that $F(x) = \begin{cases}0 & x<0\\x/6 & 0\leq x\leq 6\\ 1 &x>6\end{cases}.$
1. Compute $F'(x)$ and compare it to the normalized rug function. 
...
6. Compute the derivative of $F(x)$ and compare your answer to the normalized rug function $kf(x)$.  They should be equal.  
...
7. State $E[X]$. [If you're asking, "Did we just do this in the previous question?", then you're doing this correctly.]

### Exercises {-}

Let $f(x)=\begin{cases}2x & 0\leq x\leq 6\\ 0 &\text{otherwise}\end{cases}$. Consider a rug that lies under this function and above the $x$-axis.  Let $X$ represent the random variable that we obtain by randomly dropping a dart on the rug and recording the $x$-value. 

1. Draw the rug function $f(x)$. 
1. Use a table of centroids to explain why $E[X]=4$. 
1. Find a constant $k$ so that the area under $kf(x)$ equals 1. In other words, find the normalized rug function. 
1. Compute $F(2.5)$, and $F(4)$.  Then compute $P(2.5<X\leq 4)$. 
...
We now swap to a parabolic rug with a corresponding trap door on the ceiling above the rug. The rug is shown below, and lies below the function $f(x) = x^2$ for $0\leq x\leq 4$. We call this shape a "parabolic spandrel," language we'll need to look up areas and centroids. 
...
Download this [list of centroids of common shapes](https://www.bgu.ac.il/~yakhot/mf1/Centroids.pdf) from Engineering.com. The bottom of the first page includes a parabolic spandrel (the shape of our rug above).

1. Find the expected value of $X$ (so $E[X]$) by locating the centroid of the rug. [Solution: With $a=4$, we have $E[X] = \frac{3(4)}{4} = 3$.]
1. Use the resource above (which includes facts about area in addition to centroids) to explain why the total area of the rug is $\frac{64}{3}$. 
...
1. The fact that $P(X\leq 2) = 1/8$ means the cumulative distribution function (CDF) of $X$ has the value $F(2) = \frac{1}{8}.$ 
    a. Compute $F(1.5)$. 
    a. For $0\leq x\leq 8$, explain why $F(x) = \frac{x^3}{64}$. 
    a. If $x<0$, what is $F(x)$. 
    a. If $x>4$, state $F(x)$. 
    a. Finish by stating $F(x)$ using piecewise function notation. 
1. Give the normalized rug function (so find $k$ so that the rug under $kf(x)$ for $0\leq x\leq 4$ has area 1.) [Solution: $k = \frac{1}{A} = \frac{3}{64}$, so $kf(x) = \frac{3x^2}{64}$ for $0\leq x\leq 4$ and $kf(x) = 0$ otherwise.]
1. Compare the normalized rug function $kf(x)$ to $F'(x)$, the derivative of the cumulative distribution function. What do you notice? 

### Exercises {-}

Let $f(x)=\begin{cases}5x^2 & 0\leq x\leq 2\\ 0 &\text{otherwise}\end{cases}$. Consider a rug that lies under this function and above the $x$-axis.  Let $X$ represent the random variable that we obtain by randomly dropping a dart on the rug and recording the $x$-value. 

1. Draw the rug function $f(x)$. 
1. Use a table of centroids to explain why $E[X]=\frac{3}{2}$. 
1. Find a constant $k$ so that the area under $kf(x)$ equals 1. In other words, find the normalized rug function. 
...
In all three of the preceding examples, we started with a function $f(x)$ and considered the random variable $X$ that arose from selecting the $x$-coordinate of a point randomly chosen from the area under the function $f(x)$.  We then computed the cumulative distribution function $F(x)$ of $X$, as well as created a normalized rug function $kf(x)$ by selecting a value for $k$ so that the area under $kf(x)$ equaled 1.  In all cases, we found that the derivative of the cumulative distribution function equaled the normalized rug function, in other words $$F'(x) = kf(x).$$ In all these examples, we started with a given rug function and then computed $F(x)$ from it. In practice, we'll start with a given random variable $X$, from which we have the cumulative distribution function $F(x) = P(X\leq x)$. Taking the derivative of the CDF gives us a function $f(x) = F'(x)$ that we can think of as a normalized rug function. This leads to the following definition. 
...
1. Suppose a random variable $X$ has cumulative distribution function $F(x) = \begin{cases}0 & x<0\\x/3 & 0\leq x\leq 3\\ 1 &x>3\end{cases}.$ Compute the derivative of $F$ to obtain the probability density function of $X$.  Then draw a rug that lies under this PDF. Explain why $E[X] = 1.5$. 
1. Suppose a random variable $X$ has cumulative distribution function $F(x) = \begin{cases}0 & x<0\\x^2/16 & 0\leq x\leq 4\\ 1 &x>4\end{cases}.$ Compute the derivative of $F$ to obtain the probability density function of $X$.  Then draw a rug that lies under this PDF. Use a table of centroids to explain why $E[X] = \frac{8}{3}$. 
...
3. Find a value $k$ so that each function below is a PDF. You may need to consult a table of centroids to obtain the areas for some of these functions. 
    a. $f(x) = \begin{cases}2 & 0\leq x \leq 6\\ 0 &\text{otherwise}\end{cases}$
    a. $f(x) = \begin{cases}3x & 0\leq x \leq 4\\ 0 &\text{otherwise}\end{cases}$
    a. $f(x) = \begin{cases}5x^2 & 0\leq x \leq 2\\ 0 &\text{otherwise}\end{cases}$
...
For the triangular from the previous section, approximating the rug by using 20 rectangles instead (so $\Delta x = \frac{5-0}{20} = 0.25$), yields the exact area and an expected value of 3.33125, rather than the exact value $\mu = 3.3333\bar3$. Pretty close. If we want more accuracy, we just increase the number of rectangles in our approximation. 
...
For our parabola function, approximating the rug by using 50 rectangles yields an area approximation of 21.3312 instead of the exact area of $A = 21.333\bar 3$, and expected value approximation of 2.9997 which is only 0.0003 away from the actual value of $\mu = 3$.
...
Let's end the unit by looking at some examples that are not just rugs that lie under lines or parabolas. We can approximate almost anything with thin rectangles. For example, if our rug is the region under the function $f(x) = -x^4+2x^3-2x^2+3x+4$ for $0\leq x\leq 2$, then with 25 rectangles (show below) we get a decent approximation at the total area under $f$, as well as the $x$-coordinate of the centroid. 


```r
f <- function(x){-x^4+2*x^3-2*x^2+3*x+4}
a <- 0
b <- 2
n <- 25
dx <- (b-a)/n
...
##      0.9996626      0.3335583
```
The total exact area under the exponential distribution with PDF $f(x) = 3 e^{-3x}$ is 1 (which we approximated as $A\approx 0.9996626$) with expected value $\mu = \frac{1}{3} = 0.333\bar 3$ (which we approximated as $\mu \approx 0.3335583$). Not too bad for an approximation that resulted from just using lots of rectangles. 
...
Which kind of functions can we apply this process to? We'll restrict ourselves to a specific set of functions that help us analyze probability. We will require that $f(x)\geq 0$, as we don't want negative probabilities. In addition, we will require that the area under $f(x)$ is finite (but we may allow left and right bounds to become infinite, provided the total area under the $f(x)$ remains finite). To simplify probability computations, we'll sometimes require that the total area under our function is 1. If the area $A$ is not 1, note that letting $k=1/A$ means the area under the normalized function $kf(x)$ is 1. This leads us again to the properties that define probability density functions. 

### Exercises {-}

1. Consider a rug that lies under the function $f(x) = 4-x$ for $0\leq x\leq 4$, with corresponding random variable $X$ obtained by taking the $x$-coordinate of a randomly chosen point on the rug. The code below uses 8 rectangles to approximate the area under $f$, as well as the expected value of $X$. Increase the number of rectangles to 20, and then 100.  Then use geometric arguments to find the exact values for the area and expected value. 
...
1. Repeat the previous problem using the function $f(x) = 9-x^2$ for $0\leq x\leq 3$. 
1. Repeat the previous problem using the function $f(x) = \sqrt{25-x^2}$ for $0\leq x\leq 5$. Note that the rug is the region in the first quadrant that lies inside the circle $x^2+y^2 =25$ (so a quarter of the circle). This geometric fact will help you identify the exact values for the area and expected value from a list of known centroids.  
...
Why do we use the words probability density? The word density comes from a notion of talking about mass per length.  A thin rope made of twine has a lower mass per length than a steel wire of the same thickness. Knowing the mass per length allows us to compute the same length. 

We've connected probabilities to areas. A rug function $f(x)$ provides just the height of thin rectangles. We must multiply the height by a width $\Delta x$ in order to get a probability $p=f(x) \Delta x$. This means $f(x) = \frac{p}{\Delta x}$ is a probability $p$ per length $\Delta x$,  so a probability density.
...
1. Verify that $f(x) = \begin{cases}\frac{1}{10} & 0\leq x \leq 10\\ 0 &\text{otherwise}\end{cases}$ is a probability density function. (What two things must we check).  
    ...
    a. What is the area of a rug whose height is $\frac{1}{10}$ of a unit but whose width is 0 units?  (Do we even have a rug?)
...
* $\displaystyle\int_{0}^{5} 2 dx = 10$ (the area under a rug of height 2 for $0\leq x\leq 10$)
* $\displaystyle\int_{-2}^{3} 9-x^2 dx = \frac{100}{3}$
* $\displaystyle\int_{0}^{4} 5 e^{-5 t} dt = 1-\frac{1}{e^{10}}$ (Type "5 Exp[-5 t]" for the integrand).
...
\begin{cases}\lambda e^{-\lambda x} & x\geq 0\\ x<0\end{cases},$$ 
...
* We know that $x^2+y^2 = 9$ is a circle of radius 3. Solving for $y$ gives the top half of the circle as $y = \sqrt{9-x^2}$. This means that $\displaystyle\int_{-3}^3\sqrt{9-x^2}dx$ gives the area above the $x$-axis that lies under the top half of the circle. This is the area inside a quarter of the circle, which means $\displaystyle\int_{0}^3\sqrt{9-x^2}dx = \frac{9\pi}{4}.$
...
1. Use an area argument, rather than software or Riemann sums, to state the value of each definite integral. Check your solution with software. 
    ...
    a. $\displaystyle\int_0^7 f(x) dx$ where $f(x) = \begin{cases}3 & 0\leq x\leq 4 \\ 1 & 4< x< 7 \leq 5\end{cases}$
...
> The **expected value** of a continuous random variable $X$ with probability density function $f(x)$ is $$\mu_X = E[X] = \int_{-\infty}^{\infty} x f(x) dx.$$ 

> The **variance** of a continuous random variable$X$ with probability density function $f(x)$ is $$\sigma_X^2 = \text{Var}[X] = \int_{-\infty}^{\infty} (x-\mu)^2 f(x) dx.$$ 

> The **standard deviation** of a continuous random variable  $X$ with probability density function $f(x)$ is equal to the square root of the variance, so $\sigma_X = \sqrt{\sigma_X^2}$. 

When the context is clear, we'll drop the subscript an simply use $\mu$, $\sigma^2$, anbd $\sigma$ for the expected value, variance, and standard deviation.  

Note that for a discrete random variable $X$ where outcome $x_i$ has probability $p_i$, we defined the expected value to be $E[X] = \sum x_i p_i$. For a continous random variable with probability *density* function $f(x)$, multiplying the density $f(x)$ by a length $dx$ gives a probability. Replacing $\sum$ with $\int_a^b$, then $x_i$ with $x$, and finally $p_i$ with $f(x)dx$ shows how $$\sum x_i p_i \quad\text{becomes}\quad \underbrace{\int_a^b}_{\sum} \underbrace{x}_{x_i}\underbrace{f(x) dx}_{p_i}.$$
Using this same correspondence, we could obtain formulas for the variance and standard deviation of discrete random variables. You will encounter the expected value, variance, and standard deviation of random variables in future courses. The goal in this section is to just practice using definite integrals to compute them. 
...
1. For the function $g(x) = \begin{cases}5-x&0\leq x\leq 5\\0&\text{otherwise}\end{cases}$, we will first find a value $k$ so that $f(x) = kg(x)$ is a probability density function for a random variable $X$. We will then compute the expected value, variance, and standard deviation of $X$.  
    * A graph of $g(x)$ reveals that $g(x)\geq 0$ (one part of being a PDF). The function $g$ is zero everywhere except for $0\leq x\leq 3$, which means $$\int_{-\infty}^{\infty}g(x)dx = \int_{0}^{5}(5-x)dx = \frac{25}{2}.$$ We let $k = \frac{2}{25}$ and obtain $\displaystyle \int_{0}^{3}\frac{2}{25}(5-x)dx = 1.$ This means $f(x) = kg(x) = \begin{cases}\frac{2}{25}(5-x)&0\leq x\leq 5\\0&\text{otherwise}\end{cases}$ is a probability density function for a random variable $X$.   
    * The expected value of $X$ is $$\mu_X = E[X] = \int_{-\infty}^{\infty} x f(x) dx = \int_{0}^{5}x\frac{2}{25}(5-x)dx = \frac{5}{3}\approx 1.66667.$$
    * The variance of $X$ is $$\sigma_X^2\text{Var}[X] = \int_{-\infty}^{\infty} (x-E[X])^2 f(x) dx = \int_{0}^{5}\left(x-\frac{5}{3}\right)^2\frac{2}{25}(5-x)dx = \frac{25}{18}\approx 1.38889.$$
...
2. For the function $g(x) = \begin{cases}e^{-7x}&0\leq x < \infty\\0&\text{otherwise}\end{cases}$, we will first find a value $k$ so that $f(x) = kg(x)$ is a probability density function for a random variable $X$. We will then compute the expected value, variance, and standard deviation of $X$.  
    ...
    * The expected value of $X$ is $$\mu_X = E[X] = \int_{-\infty}^{\infty} x f(x) dx = \int_{0}^{\infty}x7e^{-7x}dx = \frac{1}{7}.$$
    * The variance of $X$ is $$\sigma_X^2\text{Var}[X] = \int_{-\infty}^{\infty} (x-E[X])^2 f(x) dx = \int_{0}^{\infty}\left(x-\frac{1}{7}\right)^27e^{-7x}dx = \frac{1}{49}.$$
    * The standard deviation of $X$ is $\sigma_X = \sqrt{\text{Var}[X]} = \sqrt{\frac{1}{49}} = \frac{1}{7}$. 
    
### Exercises {-}

1. For each function $g(x)$ below, suppose $X$ is a random variable with probability density function $f(x) = kg(x)$. Start by finding the value $k$ that makes $kg(x)$ a probability density function. Then compute the expected value $\mu$, the variance $\sigma^2$, and the standard deviation $\sigma$.   
