...
- Use the `plot()` command in R to create a time vs. bulb intensity (as a percent of the original lumens) scatter plot for one light bulb. Use the bulb data frame.
...
- Use the `plot()` and `lines()` commands in R (or the `plot()` and `points()` commands in R) to plot your each fitted function and the scatter plot of the data. You should have **6 plots**, each with one fitted function and the data. Make sure the viewing window for each plot is `xlim = c(-10,80000)` and `ylim = c(-10,120)`.
